...
function signInWithPopUpFrom3rdPartyHelper(provider: firebase.auth.GoogleAuthProvider | firebase.auth.GithubAuthProvider | firebase.auth.FacebookAuthProvider): Promise<string> {
  ...
  return firebase
    ...
    .catch(err => {
      Promise.reject(new Error(`Server Error ${String(err)}`))
    })
}

function signInHelper(type: string, credential: { email?: string, password?: string }): Promise<void> {
  return new Promise((resolve, reject) => {
    if (type === 'local' && credential.email && credential.password) {
      ...
      switch (type) {
        case 'google':
          signInWithPopUpFrom3rdPartyHelper(new firebase.auth.GoogleAuthProvider()).then(() => {
            resolve()
          })
          break
        case 'gh':
          signInWithPopUpFrom3rdPartyHelper(new firebase.auth.GithubAuthProvider()).then(() => {
            resolve()
          })
          break
        default:
...
function getTokenHelper(): Promise<string> {
  return new Promise((resolve, reject) => {
...
function updatePasswordHelper(password: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const currentUser = firebase.auth().currentUser
    if (currentUser) {
      currentUser
